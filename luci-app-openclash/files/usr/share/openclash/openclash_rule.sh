#!/bin/sh
   ...
   echo "开始下载使用中的第三方规则..." >$START_LOG
      ...
      elif [ "$RUlE_SOURCE" = "homeclash" ]; then
      	 if pidof clash >/dev/null; then
            curl -sL --connect-timeout 10 --retry 2 -x http://$PROXY_ADDR:$HTTP_PORT -U "$PROXY_AUTH" https://raw.githubusercontent.com/xiaoqingfengATGH/homeclash/master/luci-app-openclash/files/etc/openclash/homeclash.yaml -o /tmp/rules.yaml >/dev/null 2>&1
      	 else
            curl -sL --connect-timeout 10 --retry 2 https://raw.githubusercontent.com/xiaoqingfengATGH/homeclash/master/luci-app-openclash/files/etc/openclash/homeclash.yaml -o /tmp/rules.yaml >/dev/null 2>&1
         fi
         sed -i -n '/^rules:/,$p' /tmp/rules.yaml 2>/dev/null
      fi
